  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 pb-20">
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {backgrounds.map((bg) => (
          <Link key={bg.id} href={`/wiki/trasfondos/${bg.id}`} className="group relative flex min-h-[400px] overflow-hidden bg-white shadow-sm hover:shadow-2xl transition-all border-2 border-[#242528] rounded">
            ...
            <div className="absolute inset-0 flex items-center justify-end p-8 z-0 bg-muted/5">
               <div className="relative h-full w-[80%] flex items-center justify-center opacity-10 group-hover:opacity-20 transition-opacity duration-700">
                  <Scroll className="h-64 w-64 text-[#242528] rotate-12" />
               </div>
            </div>
              ...
              <div className="max-w-[85%] space-y-6">
                <div className="space-y-2">
                   <div className="flex items-center gap-3">
                      <div className="h-10 w-10 border-2 border-[#242528] rounded flex items-center justify-center bg-white group-hover:bg-foreground group-hover:border-[#242528] transition-all duration-300">
                        <BookOpen className="h-5 w-5 text-[#242528] group-hover:text-white transition-colors" />
                      </div>
                   ...
                   <p className="text-[10px] font-sans font-black text-[#242528]/40 uppercase tracking-[0.3em]">Pasado Notable</p>
                </div>
                ...
                <div className="space-y-3 pt-2">
                   <div className="flex flex-col gap-1.5">
                     <span className="text-[9px] text-[#242528]/40 font-black uppercase tracking-widest">Competencias de Habilidad</span>
                     <div className="flex flex-wrap gap-2">
                       {bg.skillProficiencies.map(skill => (
                         <Badge key={skill} variant="outline" className="border-border text-[#242528] font-black uppercase text-[9px] rounded px-2 py-0.5">{skill}</Badge>
                       ))}
                     </div>
                   </div>
                   <div className="flex flex-col gap-1.5">
                     <span className="text-[9px] text-[#242528]/40 font-black uppercase tracking-widest">Rasgo Distintivo</span>
                     <Badge className="bg-black text-white border-none uppercase text-[9px] font-black w-fit rounded px-3">{bg.feature}</Badge>
                   </div>
                </div>
      ...
      <div className="p-10 border-2 border-black bg-black text-white rounded shadow-[4px_4px_0px_0px_rgba(238,134,0,1)]">
