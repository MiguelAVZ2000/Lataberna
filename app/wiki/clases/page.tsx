  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 pb-20">
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {classes.map((clase) => (
          <Link key={clase.id} href={`/wiki/clases/${clase.id}`} className="group relative flex min-h-[450px] overflow-hidden bg-white shadow-sm hover:shadow-2xl transition-all border-2 border-[#242528] rounded">
              ...
              <div className="max-w-[80%] space-y-6">
                <div className="space-y-2">
                   <div className="flex items-center gap-3">
                      <div className="h-10 w-10 border-2 border-[#242528] rounded flex items-center justify-center bg-white group-hover:bg-foreground group-hover:border-[#242528] transition-all duration-300">
                        <Heart className="h-5 w-5 text-[#242528] group-hover:text-white transition-colors" />
                      </div>
                   ...
                   <p className="text-[10px] font-sans font-black text-[#242528]/40 uppercase tracking-[0.3em]">Arquetipo Legendario</p>
                </div>
                ...
                <div className="space-y-3 pt-2">
                   <div className="flex flex-col gap-1.5">
                     <span className="text-[9px] text-[#242528]/40 font-black uppercase tracking-widest">Habilidad Primaria</span>
                     <div className="flex flex-wrap gap-2">
                       {clase.primaryAbility.map(ability => (
                         <Badge key={ability} variant="outline" className="border-border text-[#242528] font-black uppercase text-[9px] rounded px-2 py-0.5">{ability}</Badge>
                       ))}
                     </div>
                   </div>
                   <div className="flex flex-col gap-1.5">
                     <span className="text-[9px] text-[#242528]/40 font-black uppercase tracking-widest">Dado de Vida</span>
                     <Badge className="bg-black text-white border-none uppercase text-[9px] font-black w-fit rounded px-3">D{clase.hitDie}</Badge>
                   </div>
                </div>
